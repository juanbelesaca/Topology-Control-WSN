...
for snp=1:length(snap)
    ...
    for flow=1:length(num_flow)
        filename = strcat('AllRouters/Sim0',num2str(sim),'/TotalPkts_TxRx/Rx_S',num2str(snap(snp)),'R',num2str(num_root(flow)),'F',num2str(num_flow(flow)),'.dat');
...
for snp=1:length(snap)
    temp = [];
    for flow=1:length(num_flow)
        filename = strcat('SpanningTree/Sim0',num2str(sim),'/TotalPkts_TxRx/Rx_S',num2str(snap(snp)),'R',num2str(num_root(flow)),'F',num2str(num_flow(flow)),'.dat');
        output = csvread(filename);
        temp = [temp;output'];
    end
...
for snp=1:length(snap)
    temp = [];
    for flow=1:length(num_flow)
        filename = strcat('ControlRouters/Sim0',num2str(sim),'/TotalPkts_TxRx/Rx_S',num2str(snap(snp)),'R',num2str(num_root(flow)),'F',num2str(num_flow(flow)),'.dat');
        output = csvread(filename);
        temp = [temp;output'];
    end
...
for snp=1:length(snap)
    temp = [];
    for flow=1:length(num_flow)
        filename = strcat('ControlRVaz/Sim0',num2str(sim),'/TotalPkts_TxRx/Rx_S',num2str(snap(snp)),'R',num2str(num_root(flow)),'F',num2str(num_flow(flow)),'.dat');
        output = csvread(filename);
        temp = [temp;output'];
    end
...
figure(5)
hold on
errorbar(num_root,Effic_meanCRV,CI_CRV(:,1)+1e-3,'-o','Color',[0,0.7,0])
errorbar(num_root,Effic_meanCR,CI_CR(:,1)+1e-3,'-^b')
errorbar(num_root,Effic_mean-0.05,CI(:,1)+1e-3,'-sr')
errorbar(num_root,Effic_meanST-0.025,CI_ST(:,1),'-*m')
xlim([0.9 3.1])
