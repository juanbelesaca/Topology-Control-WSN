...
for snp in range(0,len(snap)):
	...
	for flow in range(0,len(num_flows)):
		...
		out_f1 = type_sim + '/Sim0'+str(num_sim)+'/Energ_BytesTxRx/Tx_Snap'+str(snap[snp])+'Flow'+str(num_flows[flow])+'.dat'
		output_file1 = open(out_f1,'w')
		savetxt(output_file1, txBytes,delimiter=",")
		#output_file1.write(str(txMgt_Rout))
		output_file1.close()    
			
		out_f2 = type_sim + '/Sim0'+str(num_sim)+'/Energ_BytesTxRx/Rx_Snap'+str(snap[snp])+'Flow'+str(num_flows[flow])+'.dat'
