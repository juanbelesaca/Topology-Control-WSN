...
for snp in range(0, len(snap)):
	...
	for flow in range(0,len(num_flows)):
		...
		for r in range(0,num_Runs):
			...
			in_f = raiz_ns+'/Tesis/'+typeOutDir+'/Sim0'+str(num_sim)+'/Snap_'+str(snap[snp])+'/Flow_Mon/Flows'+str(num_flows[flow])+'_Run'+str(r+1)+'.xml'
		...
		out_f1 = type_sim + '/Sim0'+str(num_sim)+'/TotalPkts_TxRx/Rx_Snap'+str(snap[snp])+'Flow'+str(num_flows[flow])+'.dat'
